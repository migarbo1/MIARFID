...
function init(){
    ...
    //little camera
    if(aspectRatio > 1){
        orthographicCamera = new THREE.OrthographicCamera(-L*aspectRatio,L*aspectRatio,L,-L,-5,300);
    }else{
        orthographicCamera = new THREE.OrthographicCamera(-L,L,L/aspectRatio,-L/aspectRatio,-5,300);
    }
    orthographicCamera.position.set(0,210,0);
...
function updateAspectRatio(){
    ...
    /* update orthographic cameras view */
    if(ar > 1){
        L = window.innerHeight/4
        orthographicCamera.left = -L*newAspectRatio;
        orthographicCamera.right = L*newAspectRatio;
        orthographicCamera.top = L;
        orthographicCamera.bottom = -L;

    }else{
        L = window.innerWidth/4
        orthographicCamera.left = -L;
        orthographicCamera.right = L;
        orthographicCamera.top = L/newAspectRatio;
        orthographicCamera.bottom = -L/newAspectRatio;
    }
