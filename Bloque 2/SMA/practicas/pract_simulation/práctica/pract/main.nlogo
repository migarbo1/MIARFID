...
globals [ exits-list exit-1-x exit-1-y exit-2-x exit-2-y exit-3-x exit-3-y exit-4-x exit-4-y total in-danger people-safe student-dead professors-dead fires-shut]
...
to initialize-statistics
  set total student-count + professor-count
  set in-danger total
  set people-safe 0
  set student-dead 0
  set professors-dead 0
  set fires-shut 0
end
...
to move-students
  ask students with [safe? = false and finish-state? = false]
  [
...
to shut-up-fire
  ...
  [
    let fire min-one-of (fire-spots in-radius 1 with [fire? = true]) [distance myself]
    if fire != nobody
    [
...
to check-corpses
  ...
  ask professors with [health-professor <= 0]
  [
    set professors-dead professors-dead + 1
    die
  ]
end
...
to move-towards-fire
  ...
  ask professors with [health-professor > 0]
  [
    let target-patch min-one-of (patches in-radius 10 with [fire? = true]) [distance myself]
...
"" ""
PENS
"in-danger" 1.0 0 -1184463 true "" "plot in-danger"
"safe" 1.0 0 -13840069 true "" "plot people-safe"
"student-dead" 1.0 0 -2674135 true "" "plot student-dead"
"professors-dead" 1.0 0 -2064490 true "" "plot professors-dead"
"fires-shut" 1.0 0 -955883 true "" "plot fires-shut"
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>student-dead</metric>
    <enumeratedValueSet variable="fire-harm">
      <value value="75"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="num_fires">
      <value value="5"/>
    </enumeratedValueSet>
  </experiment>
