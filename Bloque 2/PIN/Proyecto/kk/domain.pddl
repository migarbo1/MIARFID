...
(define (domain puerto)
	(:requirements :strips :equality :typing :fluents :conditional-effects)
	(:types 
		location container stack rail arm num - object
		ascendent descendent - rail
	)
	(:predicates
		(next ?n1 - num ?n2 - num)
	...
	(:action stackOnContainer
		...
		:effect (and 
			...
			(when 
				(is_target ?t) ;;condition
				(available ?t) ;;action
			)
			(when 
				(and (is_regular ?t) (is_target ?c))
				(not(available ?c))
			)
	...
	(:action unstack
		...
		:effect (and 
			...
			(not (clear ?t))
			(not (free ?a))
			(when (is_target ?t) (not (available ?t)))
